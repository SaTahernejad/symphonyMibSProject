		     ...
		     char branching, double *heur_solution)
{
	...
	*feasible = IP_FEASIBLE;
	...
	double *VarVal = new double[prob->numvars]();
	...
	int i1(0);
        double *rhs1;
        rhs1 = (double *) malloc(prob->m * DSIZE);
 ...
 /* Load the problem to SYMPHONY */
	sym_explicit_load_problem(env2, prob->n, prob->m, prob->column_starts, prob->matrix_indices,
	...
	sym_warm_solve(env2);
        double  objval;
	sym_get_obj_val(env2,&objval);
	if(fabs(objval-low_objval2) > lpetol){
	    *feasible = IP_INFEASIBLE;
	}
	delete [] VarVal;
	return(USER_SUCCESS);
...
//Note:Does cut work for ISMP?
      ...
      //int  * indexList = new int[numCols];
      int  *indexList = (int *) malloc((numCols) * ISIZE); 
      //double * valsList = new double[numCols];
